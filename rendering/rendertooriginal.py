"""
Multi-model car renderer -- ORIGINAL-.glb variant.

Same pipeline as optimize_render_car_multimodel.py (fit all four box templates,
keep the best), but the DISPLAY mesh is the original downloaded vehicle model in
models/<type>.glb (e.g. the photoreal BMW X5), stretched to the predicted
OW/OH/OL bounding box via vehicle_meshes.render_arrays().

Difference from optimize_render_car_multimodel.py:
  * that file uses vehicle_meshes.reconstruct() -> the Kenney kit with wheels
    relocated to the derived wheelbase/track (6 params, models/kenney/).
  * THIS file uses vehicle_meshes.render_arrays() -> the original models/*.glb,
    rigid template scaled only by OW/OH/OL (3 params).

Run:  python rendertooriginal.py [row_index]
...
class GLWidget(QOpenGLWidget):
    def __init__(self, points_file, pred_ow, pred_wb, pred_ol, pred_oh,
        ...
        # this only replaces what's drawn).  ORIGINAL .glb: rigid template scaled
        # only to the predicted OW/OH/OL bounding box (no wheel relocation).
        self.car_list = None
        try:
            self.mesh = vehicle_meshes.render_arrays(
                best, ow=self.params[8], oh=self.params[6], ol=self.params[7])
            self.use_mesh = True
        except Exception as exc:
            print(f"[mesh] original model unavailable, drawing box instead: {exc}")
            self.use_mesh = False
    ...
    def renderCar(self):
        if self.use_mesh and self.car_list is not None:
            glCallList(self.car_list)            # original .glb model
