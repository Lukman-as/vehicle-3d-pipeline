"""
Original parametric POLYGON car renderer.

This is the "boxes + cylinders" model from the design doc (Sketch-up / Model Car
Vertices): a watertight polygon body whose shape is driven entirely by the 11
derived dimensions -- no downloaded .glb meshes.  For each detected car it fits
all four box templates, keeps the best type, and draws that type's polygon mesh.

How the 11 parameters reshape the mesh (built in car_models_core.build_geometry,
following the doc's construction; see Figures 1-4 and the draft on page 5):
    OL, OW, OH  -> overall length / width / height (the bounding envelope)
    A           -> hood length: windshield base sits at z = OL/2 - A
    C           -> beltline drop: bottom of the side glass at y = OH - C
    D           -> body (door) height: rocker/sill at y = OH - C - D
    E           -> greenhouse width: cabin inset = (OW - E)/2 per side
    F, G        -> front / rear axle positions (front at OL/2 - F, rear at G - OL/2)
    TW          -> track width: wheels at x = +-TW/2  (tire width = OW - TW)
    WB          -> wheelbase; redundant with F,G,OL so it conditions the Gaussian
                   upstream rather than being placed directly.

The mesh is watertight and the targets stay ordered, so it never "breaks
structure" the way a stretched real model can.

Run:  python originalrender.py [row_index]
      python originalrender.py --cam sc1 --frame 0035 --obj 16
(reads data/gen/matlab_output directly via render_io.py)
"""
...
import car_models_core_new as core
...
class GLWidget(QOpenGLWidget):
    def __init__(self, points_file, pred_ow, pred_wb, pred_ol, pred_oh,
        ...
        # ---- load + transform the point cloud (identical to the other renderers) ----
        ...
        self.params = results[best]['params']          # the 11 derived dimensions
        self.geometry = results[best]['geom']          # polygon mesh from those params
    ...
    # ------------------------------------------------------------- rendering
    def renderCar(self):
        """Draw the parametric polygon body (quads) + tires (cylinders)."""
