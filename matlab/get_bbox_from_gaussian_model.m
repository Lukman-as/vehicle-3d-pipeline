function [get_length_gaussian, corners] = get_bbox_from_gaussian_model(all_3D_points,all_annos, fieldname,car_model_param,id_to_tire, mirror_one_side)
    wheelbase = all_annos.(fieldname).wheelbase;
    overall_width = all_annos.(fieldname).w_unit;
%     overall_height = norm(all_annos.(fieldname).pred_bbox_points_only(:,1) - all_annos.(fieldname).pred_bbox_points_only(:,5));
    use_height = false;
    if wheelbase ~= -1
%         if use_height
...
%            b_vector = [b_vector overall_height*100];
   ...
   for i=1:size(a_vars,1)
    ...
    if overall_height <=0
        overall_height = max(abs(min(all_3D_points(2,:))),1.8); %Prevent negative 
    % case only, points in the ground or multivariate model cannot output
    % height correctly.
    end
    assert(overall_height > 0, 'Height gotta be larger than 0');
    extreme_height_point = [0 -overall_height 0 1]';
    
  
    all_3D_points = [all_3D_points, extreme_front_point, extreme_rear_point extreme_height_point];
    length_based_on_gaussian = abs(extreme_front_point(1) - extreme_rear_point(1));
    length_based_on_points = max(all_3D_points(1,:)) - min(all_3D_points(1,:)); %After considering the points
    if length_based_on_gaussian > length_based_on_points %Aka contain all the points inside, no extension
        get_length_gaussian = 1;
    else
        get_length_gaussian = 0;
    end
    corners = get_bbox_from_points(all_3D_points, false,all_annos.(fieldname).has_mirror,...
        mirror_one_side,all_annos.(fieldname).w_unit);
end