function cMw = get_cMw_using_car_points(k_matrix,R,vp_car, origin_sym_coor,all_annos,annotated_car_id, all_3D_points_car)
    sym_normal_unit = flip_sym_normal_vpcar(k_matrix,R,all_annos,annotated_car_id,vp_car);
...
% 
    ...
    %Inverse back to world coor to compare
    estimated_3d_points = inv(cMw)*all_3D_points_car;
    estimated_3d_points = estimated_3d_points(1:3,:);
    start_pos = size(all_annos.(fieldname).tire_ids,2);

    ex_3D_1 = estimated_3d_points(:,start_pos+1);
    ex_3D_2 = estimated_3d_points(:,start_pos+2);
    ...
    assert(abs(rad2deg(ex_sym_angle)) <= 10, 'Always valid');
%     if abs(ex_sym_angle) > 1e-3
%         rad2deg(ex_sym_angle)
%         'if flipped'
%         rad2deg(acos(dot(ex_vec, -sym_normal_unit)))
%     end
%     assert(abs(ex_sym_angle) < 1e-3, 'Always valid');
% 
