function tground_3D = get_refined_back_v2(k_matrix,R,plane,...
    mounting_height,annotated_tgs,image,tground_centroid)
    near_tground = annotated_tgs(:,1);
    near_tground_back = annotated_tgs(:,2);
    d_plane = plane(4);
    sym_normal_unit = plane(1:3);
    ng = [0,1,0]';
    nl = sym_normal_unit;
    u = cross(nl, ng);
    u = u/norm(u);
    ...
    world_to_ground = [1 0 0; 0 0 1; 0 1/mounting_height 0];
...
%   DEBUG purpose
    ...
    lambda_tg = mounting_height/left_tire_refined_2D_proj(2);
    ...
    t_ground_3D_2D = k_matrix*R*tground_3D;
    t_ground_3D_2D = t_ground_3D_2D./t_ground_3D_2D(3,:);
    ...
    if false
        ...
        imshow(image)
        hold on
        scatter(near_tground(1,:),near_tground(2,:),20,'r','filled');
        scatter(near_tground_back(1,:),near_tground_back(2,:),20,'r','filled'); 
        scatter(t_ground_3D_2D(1,:),t_ground_3D_2D(2,:),20,'green','filled'); 
